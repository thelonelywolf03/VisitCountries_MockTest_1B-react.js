...
class App extends Component {
  ...
  changeVisit = id => {
    ...
    const updatedData = countriesList.map(each => {
      if (each.id === id) {
        return {...each, isVisited: !each.isVisited}
      }
      return {...each}
    })
    this.setState({countriesList: updatedData})
  }
    ...
    return (
      <div className="bg-cont">
        ...
        {visitedCountries.length === 0 ? (
          <p className="no">No Countries Visited Yet!</p>
        ) : (
          <ul className="list-countries">
            {visitedCountries.map(each => (
              <VisitedCountries
                item={each}
                key={each.id}
                removeVisited={this.changeVisit}
              />
            ))}
          </ul>
        )}
